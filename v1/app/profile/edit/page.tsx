...
export default function EditProfilePage() {
  const { user, loading, login } = useAuth() // login 함수를 사용하여 사용자 정보 새로고침
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Only send nickname for now as per backend API
      await api.patch("/api/user/me", { nickname: formData.nickname })
      // Re-fetch user data to update context
      const token = localStorage.getItem("access")
      if (token) {
        await login(token) // This will re-fetch user data and update context
      }
      alert("프로필이 성공적으로 업데이트되었습니다!")
      router.push("/profile")
    } catch (err: any) {
      console.error("Profile update failed:", err)
      setError(err.response?.data || "프로필 업데이트 중 오류가 발생했습니다.")
    } finally {
      setIsSubmitting(false)
    }
  }

  // Other settings tabs (notifications, privacy, security) are not in scope for this task
  // and will retain their mock data/dummy handlers.
                  ...
                  <div className="flex flex-col items-center space-y-4">
                    <div className="relative">
                      <Avatar className="w-24 h-24">
                        ...
                        />
                        <AvatarFallback className="text-2xl">{formData.nickname ? formData.nickname[0] : user.username[0]}</AvatarFallback>
                      </Avatar>
                      <Button
                        type="button"
                        size="sm"
